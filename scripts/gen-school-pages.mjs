#!/usr/bin/env node
/**
 * Generate school profile .astro pages from schools.json.
 * Handles TWO fee formats:
 *   A) Tanglin-style: { year, tuition, buildingFund, total }
 *   B) Standard: { yearGroup, annualFee, currency }
 */
...
const CURRENCY_INFO = {
  SGD: { rate: 0.7777, label: 'SGD', desc: '1 SGD = 0.7777 USD' },
  HKD: { rate: 0.1282, label: 'HKD', desc: '1 HKD = 0.1282 USD' },
  THB: { rate: 0.0291, label: 'THB', desc: '1 THB = 0.0291 USD' },
  MYR: { rate: 0.2326, label: 'MYR', desc: '1 MYR = 0.2326 USD' },
  JPY: { rate: 0.0067, label: 'JPY', desc: '1 JPY = 0.0067 USD' },
  CNY: { rate: 0.1380, label: 'CNY', desc: '1 CNY = 0.1380 USD' },
  VND: { rate: 0.0000392, label: 'VND', desc: '1 VND = 0.0000392 USD' },
  IDR: { rate: 0.0000610, label: 'IDR', desc: '1 IDR = 0.0000610 USD' },
};

for (const school of schools) {
  if (school.slug === 'tanglin-trust-school') continue;

  const { slug, countrySlug, citySlug, country, city, feeCurrency } = school;
  const cur = CURRENCY_INFO[feeCurrency] || { rate: 1, label: feeCurrency, desc: `1 ${feeCurrency}` };

  // Detect fee format
  const hasFees = school.fees && school.fees.length > 0;
  const isTanglinFormat = hasFees && school.fees[0].hasOwnProperty('tuition');

  // Build fee table section based on format
  let feeTableSection;
  if (!hasFees) {
    feeTableSection = '';
  } else if (isTanglinFormat) {
    // Same format as tanglin-trust-school.astro
    feeTableSection = `
  <!-- FEE TABLE -->
  {school.fees && school.fees.length > 0 && (
    <section class="section">
      <div class="bsa-container">
        ...
        <p class="font-body text-black/50 text-sm mb-8">All fees in ${cur.label} per year. USD equivalent shown at ${cur.desc}.</p>
          ...
          <table class="w-full text-left max-w-2xl">
            <thead>
              <tr class="border-b border-black">
                ...
                <th class="label py-3 pr-4 text-right">Tuition (${cur.label})</th>
                <th class="label py-3 pr-4 text-right hidden sm:table-cell">Building Fund</th>
                <th class="label py-3 text-right">Total (${cur.label})</th>
    ...
    // Standard format: { yearGroup, annualFee, currency }
    feeTableSection = `
  <!-- FEE TABLE -->
  {school.fees && school.fees.length > 0 && (
    <section class="section">
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-left max-w-2xl">
            <thead>
              <tr class="border-b border-black">
                <th class="label py-3 pr-4">Year Group</th>
                <th class="label py-3 pr-4 text-right">Annual Fee (${cur.label})</th>
                <th class="label py-3 text-right hidden md:table-cell">Annual Fee (USD)</th>
              </tr>
            </thead>
            <tbody>
              {school.fees.map((fee) => (
                <tr class="border-b border-border">
                  <td class="py-3 pr-4 font-body text-sm">{fee.yearGroup}</td>
                  <td class="py-3 pr-4 font-body text-sm text-right font-medium">{formatFee(fee.annualFee)}</td>
                  <td class="py-3 font-body text-sm text-right hidden md:table-cell text-black/60">{toUSD(fee.annualFee)}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
const relatedSchools = schools
  .filter((s) => s.citySlug === '${citySlug}' && s.slug !== school.slug)
...
function toUSD(local: number): string {
  return Math.round(local * ${cur.rate}).toLocaleString('en-US');
}
---
<BaseLayout
...
>
  ...
  <section class="relative w-full h-[40vh] min-h-[300px] overflow-hidden">
    <img
      src={school.heroImage || '/images/${citySlug}-hero.jpg'}
      alt={school.name}
  ...
  <div class="bsa-container pt-6 pb-4">
    <nav class="font-body text-xs text-black/50" aria-label="Breadcrumb">
      ...
      <a href="/countries/${countrySlug}" class="hover:text-black">${country}</a>
      <span class="mx-1">/</span>
      <a href="/cities/${citySlug}" class="hover:text-black">${city}</a>
      <span class="mx-1">/</span>
...
`;
  ...
  const dirPath = join(BASE, 'src/pages/schools', countrySlug, citySlug);
  mkdirSync(dirPath, { recursive: true });
  const filePath = join(dirPath, `${slug}.astro`);
  writeFileSync(filePath, page);
  console.log(`Created: schools/${countrySlug}/${citySlug}/${slug}.astro`);
}

console.log('\nDone! Regenerated all school profile pages.');
